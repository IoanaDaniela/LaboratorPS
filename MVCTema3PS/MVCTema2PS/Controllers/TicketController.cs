...
namespace MVCTema2PS.Controllers
{
    public class TicketController : Controller
    {
        ...
        public ActionResult JSONExport(int showId = 0)
        {
            if (Session["UserRole"].Equals("Employee"))
            {
                Show show = showDB.Shows.Find(showId);
                ...
                if (tickets.Count() > 0)
                {
                    ExportFactory expFact = ExportFactory.getInstance();
                    Export e = expFact.export(ExportFactory.ExportTypes.JSON);


                    Response.Write(e.export(tickets));
                    Response.ContentType = "application/txt";
                    Response.AppendHeader("Content-Disposition", "attachment; filename=tickets.txt");
                    Response.End();
                }
        ...
        public ActionResult CSVExport(int showId = 0)
        {
            if (Session["UserRole"].Equals("Employee"))
            {
                Show show = showDB.Shows.Find(showId);
                ...
                {
                    ExportFactory expFact = ExportFactory.getInstance();
                    Export e = expFact.export(ExportFactory.ExportTypes.CSV);
                    Response.Write(e.export(tickets));
                    Response.ContentType = "application/txt";
                    Response.AppendHeader("Content-Disposition", "attachment; filename=tickets.csv");
                    Response.End();
